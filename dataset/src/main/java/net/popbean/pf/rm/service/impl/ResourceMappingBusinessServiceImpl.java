...
public class ResourceMappingBusinessServiceImpl extends AbstractBusinessService implements ResourceMappingBusinessService {
	@Autowired
	...
	public JSONObject fetchRangesForMapping(String rm_code, String pk_subject, SecuritySession client)throws BusinessError {
		try {
			...
			String pk_key = ds_inst.pk_field;
...
//				list = sub(range_list,rlt_list,pk_key);
	...
	private List<JSONObject> fetchResourceList(String rm_unique,JSONObject param,List<JSONObject> list,SecuritySession client)throws BusinessError{
		try {
			...
			List<JSONObject> ret =fetchResourceList(rm_inst,pk_list,param,client);
			if(list == null){
				//说明是第一次或者不需要合并
			}else{
				String pk_resource_code = rm_inst.resource_ds_model.code;
				DataSetModel resource_ds_model = dsService.findModel(pk_resource_code,null,false,false,null, client);
	...
	public List<JSONObject> fetchSubjectList(String rm_unique,JSONObject param,Paging paging,SecuritySession client)throws BusinessError{
		try {
			...
			DataSetModel model = dsService.findModel(pk_subject_code, param, true,true, paging, client);
			List<JSONObject> ret = model.data; 
			return ret;
		} catch (Exception e) {
			processError(e);
		}
...
//				list = intersect(range_list, rlt_list, pk_key,"PK_RESOURCE");
	...
	public ResourceMappingModel findModel(String rm_code,SecuritySession client)throws BusinessError{
		try {
			...
			DataSetModel resource_model = dsService.findModel(pk_resource, JO.gen(), false, false, null, client);
			rm_inst.subject_ds_model = subject_model;
			rm_inst.resource_ds_model = resource_model;
			return rm_inst;
		} catch (Exception e) {
			processError(e);
		}
	...
	public JSONObject syncing(String rm_code, String pk_subject, List<JSONObject> pk_subjects, SecuritySession client) throws BusinessError {
		...
		try {
			...
			rlt_md_code = rmmodel.relation_code;
