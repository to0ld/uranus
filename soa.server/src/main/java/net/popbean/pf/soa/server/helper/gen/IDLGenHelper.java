package net.popbean.pf.soa.server.helper.gen;
...
import net.popbean.pf.soa.server.helper.gen.domain.Generic;
import net.popbean.pf.soa.server.helper.gen.domain.ThriftEnum;
import net.popbean.pf.soa.server.helper.gen.domain.ThriftEnumField;
import net.popbean.pf.soa.server.helper.gen.domain.ThriftField;
import net.popbean.pf.soa.server.helper.gen.domain.ThriftIDLStruct;
import net.popbean.pf.soa.server.helper.gen.domain.ThriftMethod;
import net.popbean.pf.soa.server.helper.gen.domain.ThriftMethodArg;
import net.popbean.pf.soa.server.helper.gen.domain.ThriftService;
import net.popbean.pf.soa.server.helper.gen.domain.ThriftStruct;
import net.popbean.pf.soa.server.helper.gen.domain.ThriftType;
...
public class IDLGenHelper {
	...
	public static String gen(Class<?> clazz) {
		ThriftIDLStruct idl = build(clazz);
		StringBuilder ns = new StringBuilder("namespace java "+idl.packageName);//namespace java ${thriftServicePackage}
		StringBuilder enum_sb = new StringBuilder();
		for(ThriftEnum te:idl.enums){
			enum_sb.append("enum "+te.name+" {");
			int pos = 0;
			for(ThriftEnumField tef : te.fields){
				if((pos++)!=0){
					enum_sb.append(",");
				}
				enum_sb.append(tef.name+" = "+tef.value);
			}
			enum_sb.append("}");
		}
		StringBuilder struct_sb = new StringBuilder();
		for(ThriftStruct ts :idl.structs){
			struct_sb.append("struct "+ts.name+" {");
			int pos =1;
			for(ThriftField tf : ts.fields){
				if(pos !=1){
					struct_sb.append(",");
				}
				struct_sb.append((pos++)+":"+tf.genericType.toThriftString()+" "+tf.name);
			}
			struct_sb.append("}");
		}
		StringBuilder service_sb = new StringBuilder();
		for(ThriftService ts : idl.services){
			service_sb.append("service "+ts.name+" {");
			int tm_pos = 1;
			for(ThriftMethod tm : ts.methods){
				if((tm_pos++)!=1){
					service_sb.append(",");
				}
				service_sb.append(tm.returnGenericType.toThriftString()+" "+tm.name+"(");
				int pos =1;
				for(ThriftMethodArg tma: tm.methodArgs){
					if(pos !=1){
						service_sb.append(",");
					}
					service_sb.append((pos++)+":"+tma.genericType.toThriftString()+" "+tma.name);
				}
				service_sb.append(")");
			}
		...
		return new StringBuilder().append(ns).append("\n").append(enum_sb).append("\n").append(struct_sb).append("\n").append(service_sb).append("\n").toString();
	}

	public static ThriftIDLStruct build(Class<?> clazz) {
		// 将一个service接口分为四部分：package,enum,struct,service
		ThriftIDLStruct ret = new ThriftIDLStruct(clazz);
		ret.packageName = getPackageName(clazz);
		//enum:非常特殊，需要将struct，service都处理完才能轮到它
		//FIXME ret = buildThriftService(ret)
		ret = buildThriftService(ret);
		return ret;
	}
	 ...
	 * @return
	 */
	private static ThriftIDLStruct buildThriftService (ThriftIDLStruct idl){
		ThriftService service = new ThriftService();
		System.out.println("service.name:"+idl.clazz.getName());
		service.name = idl.clazz.getSimpleName();
		idl.services = Arrays.asList(service);
		//
		Method[] methods = idl.clazz.getDeclaredMethods();
		for (Method method : methods) {
			idl = buildThriftStruct(method, idl);
			//
			ThriftMethod thriftMethod = new ThriftMethod();
			idl.services.get(0).methods.add(thriftMethod);
			//
			thriftMethod.name = method.getName();
			thriftMethod.relationClasses = getMethodReturnTypeRelationClasses(method);
			
			ThriftType returnThriftType = ThriftType.fromJavaType(method.getGenericReturnType());
			thriftMethod.returnGenericType = Generic.fromType(method.getGenericReturnType());
			if(returnThriftType.isStruct()) {
				idl = buildThriftStruct(method.getReturnType(), idl);
			}
			
			Type[] paramTypes = method.getGenericParameterTypes();
			String[] paramNames = lookupParameterNames(method,false);
			idl = buildThriftMethodArgs(paramTypes, paramNames,idl);
			
//			thriftMethod.methodArgs = methodArgs;
//			thriftMethods.add(thriftMethod);
	...
	public static ThriftIDLStruct buildThriftStruct(Method method, ThriftIDLStruct idl) {
		
		List<Class<?>> classes = getMethodReturnTypeRelationClasses(method);//返回值的类型MyList<K,V>的话,就得是3个
		for (Class<?> class1 : classes) {
			//A.B.C这种变态的情况
			idl = buildThriftStruct(class1, idl);
//			idl.structs.add(thriftStructBuilder.);
		}
		
		Class<?> returnClass = method.getReturnType();
		ThriftType thriftType = ThriftType.fromJavaType(returnClass);
		if(thriftType.isStruct()) {
			idl = buildThriftStruct(returnClass, idl);
		}
		
		Class<?>[] argClasses = method.getParameterTypes();
		List<Class<?>> methodGenericTypes = new ArrayList<Class<?>>();
		List<Class<?>> paramTypes = getMethodGenericParameterTypes(method);
		for (Class<?> paramType : paramTypes) {
			getGenericParameterTypes(paramType, methodGenericTypes);
		}
		for (Class<?> genericType : methodGenericTypes) {
			...
			idl = buildThriftStruct(genericType,idl);
		}
		...
		for (Class<?> clazz : argClasses) {
			...
			idl = buildThriftStruct(clazz,idl);
		}
	 ...
	 * @return
	 */
	public static ThriftIDLStruct buildThriftStruct(Class<?> clazz, ThriftIDLStruct idl) {
		Field[] fields = clazz.getDeclaredFields();
		ThriftStruct struct = new ThriftStruct();
		struct.name = clazz.getSimpleName();
		idl.structs.add(struct);
		//
		for (Field field : fields) {
			idl = buildThriftField(field, idl);
		}		
		return idl;
	}
	public static ThriftIDLStruct buildThriftField(Field field, ThriftIDLStruct idl) {
		ThriftField thriftField = new ThriftField();
		thriftField.name = field.getName();
		if(field.getName().equals("__PARANAMER_DATA")) {
			return null;
		}
		Type type = field.getGenericType();
		Generic generic = Generic.fromType(field.getGenericType());
		thriftField.genericType = generic;
		if(type instanceof ParameterizedType) {
			idl = buildStrutsByGeneric(generic, idl);
		}else {
			if(generic.isEnum() || generic.isStruct()) {
				
				generic.javaClass = field.getType();
				generic.value = field.getType().getSimpleName();
				generic.javaTypeName = field.getType().getSimpleName();
				if(generic.isStruct()) {
					idl = buildThriftStruct(field.getType(), idl);
				}else {
					idl.enums.add(buildThriftEnum(field.getType()));
				}
			}
		}
		idl.mergeField(thriftField);//将thriftfield放到最后一个struct上
		return idl;
	}
	public static ThriftIDLStruct buildThriftMethodArgs(Type[] paramTypes, String[] paramNames, ThriftIDLStruct idl) {
		for (int i = 0; i < paramTypes.length; i++) {
			ThriftMethodArg methodArg = new ThriftMethodArg();
			methodArg.name = (paramNames == null || paramNames.length == 0 ? ("arg" + i) : paramNames[i]);

			Type paramType = paramTypes[i];
			ThriftType paramThriftType = ThriftType.fromJavaType(paramType);
			methodArg.genericType = Generic.fromType(paramType);
			if(paramThriftType.isStruct()) {
				idl = buildThriftStruct((Class<?>)paramType, idl);
			}
			idl.mergeMethodArg(methodArg);
		}
		return idl;
	}
	public static ThriftIDLStruct buildStrutsByGeneric(Generic generic, ThriftIDLStruct idl) {
		List<ThriftType> thriftTypes = (List<ThriftType>)generic.types;
		for (ThriftType subThriftType : thriftTypes) {
			if(subThriftType.isStruct()) {
			    idl = buildThriftStruct(subThriftType.javaClass, idl);
			}
			if(subThriftType instanceof Generic) {
				idl = buildStrutsByGeneric((Generic) subThriftType, idl);
			}
		}
		return idl;
	}
	private static ThriftEnum buildThriftEnum(Class<?> clazz) {
		Field[] fields = clazz.getDeclaredFields();
		ThriftEnum thriftEnum = new ThriftEnum();
		thriftEnum.name = clazz.getSimpleName();
    ...
    public static String[] lookupParameterNames1(AccessibleObject methodOrCtor, boolean throwExceptionIfMissing) {
        Class<?>[] types = null;
        ...
        if (byteCodeStream == null) {
            if (throwExceptionIfMissing) {
                throw new RuntimeException("Unable to get class bytes");//ParameterNamesNotFoundException("Unable to get class bytes");
            } else {
                return EMPTY_NAMES;
            }
        }
...
//                throw new ParameterNamesNotFoundException("IoException while reading class bytes", e);
            	throw new RuntimeException("IoException while reading class bytes");
            } else {
                return EMPTY_NAMES;
            }
        }
    ...
    private static InputStream getClassAsStream(ClassLoader classLoader, String className) {
        ...
        if (asStream == null) {
            asStream = IDLGenHelper.class.getResourceAsStream(name);
        }
        return asStream;
    }
	...
	private static class TypeCollector {
        ...
        public MethodCollector visitMethod(int access, String name, String desc) {
            ...
            MyType[] argumentTypes = MyType.getArgumentTypes(desc);
            int longOrDoubleQuantity = 0;
            for (MyType t : argumentTypes) {
        ...
        private String correctTypeName(MyType[] argumentTypes, int i) {
	...
	private static class MyType {
         ...
         * The <tt>void</tt> type.
         */
        private final static MyType VOID_TYPE = new MyType(VOID, null, ('V' << 24) 
            | (5 << 16) | (0 << 8) | 0, 1);

        /**
         * The <tt>boolean</tt> type.
         */
        private final static MyType BOOLEAN_TYPE = new MyType(BOOLEAN, null, ('Z' << 24) 
            | (0 << 16) | (5 << 8) | 1, 1);

        /**
         * The <tt>char</tt> type.
         */
        private final static MyType CHAR_TYPE = new MyType(CHAR, null, ('C' << 24) 
            | (0 << 16) | (6 << 8) | 1, 1);

        /**
         * The <tt>byte</tt> type.
         */
        private final static MyType BYTE_TYPE = new MyType(BYTE, null, ('B' << 24) 
            | (0 << 16) | (5 << 8) | 1, 1);

        /**
         * The <tt>short</tt> type.
         */
        private final static MyType SHORT_TYPE = new MyType(SHORT, null, ('S' << 24) 
            | (0 << 16) | (7 << 8) | 1, 1);

        /**
         * The <tt>int</tt> type.
         */
        private final static MyType INT_TYPE = new MyType(INT, null, ('I' << 24) 
            | (0 << 16) | (0 << 8) | 1, 1);

        /**
         * The <tt>float</tt> type.
         */
        private final static MyType FLOAT_TYPE = new MyType(FLOAT, null, ('F' << 24) 
            | (2 << 16) | (2 << 8) | 1, 1);

        /**
         * The <tt>long</tt> type.
         */
        private final static MyType LONG_TYPE = new MyType(LONG, null, ('J' << 24) 
            | (1 << 16) | (1 << 8) | 2, 1);

        /**
         * The <tt>double</tt> type.
         */
        private final static MyType DOUBLE_TYPE = new MyType(DOUBLE, null, ('D' << 24) 
            | (3 << 16) | (3 << 8) | 2, 1);
         ...
         */
        private MyType(final int sort, final char[] buf, final int off, final int len) {
            this.sort = sort;
            this.buf = buf;
            this.off = off;
            this.len = len;
         ...
         *         method descriptor.
         */
        private static MyType[] getArgumentTypes(final String methodDescriptor) {
         ...
         */
        private static MyType getType(final char[] buf, final int off) {
            int len;
            switch (buf[off]) {
                case 'V':
                    return VOID_TYPE;
                ...
                case '[':
                    ...
                    return new MyType(ARRAY, buf, off, len + 1);
                    // case 'L':
                default:
                    len = 1;
                    while (buf[off + len] != ';') {
                    ...
                    return new MyType(OBJECT, buf, off + 1, len - 1);
            }
        }
